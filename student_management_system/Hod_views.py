...
from app.models import Course,Session_Year,Student,Student_Feedback,Staff,Subject,Staff_Notification,Staff_Leave,Staff_Feedback,Student_Notification,Student_Leave,Attendance_Report,Attendance,StudentResult
# for showing alert message we are importing beleow module 
...
def ADD_STAFF(request):
    if request.method == "POST":
        ...
        if CustomUser.objects.filter(username=username).exists():
            ...
            staff.save()
            messages.success(request,'Staff is Sucessfully Added !')
            return redirect('add_staff')
...
def ADD_SUBJECT(request):
    ...
    if request.method == "POST":
        ...
        subject.save()
        messages.success(request,'Subject is Suceessfully Added !')
...
def SAVE_STAFF_NOTIFICATION(request):
    if request.method == "POST":
        ...
        notification.save()
        messages.success(request,'notifcation is successfully sent !')
...
def Staff_Feedbacks(request):
    ...
    feeedback_history = Staff_Feedback.objects.all().order_by('-id')[0:5]

    context = {
        'feedback':feedback,
        'feeedback_history':feeedback_history,
    }
...
def Staff_Feedbacks_save(request):
    if request.method == "POST":
...
def Students_Feedbacks(request):

    feedback =  Student_Feedback.objects.all()
    feeedback_history = Student_Feedback.objects.all().order_by('-id')[0:5]
...
def Students_Feedbacks_save(request):
    if request.method == "POST":
        feedback_id = request.POST.get('feedback_id')
        feedback_reply = request.POST.get('feedback_reply')
